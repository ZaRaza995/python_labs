...
def write_csv(
    rows: Sequence[Sequence], path: str | Path, header: Sequence[str] | None = None
) -> None:
    ...
    if rows:
        ...
        if not all(
            len(row) == expected_len for row in rows
        ):  # Проверка остальных строк
            raise ValueError(
                "Все строки для записи в CSV должны иметь одинаковую длину."
            )
